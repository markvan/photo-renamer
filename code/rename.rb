...
class Layout
  ...
  def initialize(width, height)
    ...
    @prev_button = TkButton.new(@root) { text 'prev'; command proc { Layout.image_viewer.previous_image } }
    @next_button = TkButton.new(@root) { text 'next'; command proc { Layout.image_viewer.next_image } }
  ...
  def layout
    ...
    @@image_viewer.tk_lable.grid('row' => 4, 'column' => 0, 'columnspan' => 2, 'pady' => 25)
  end
end
